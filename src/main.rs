use hashmap::{ast_to_hashmap, PklTable, PklValue};
use lexer::PklToken;
use logos::Logos;
use parser::{parse_pkl, ParseError, PklResult};
...
impl<'a> Pkl<'a> {
    ...
    pub fn parse(&mut self) -> PklResult<()> {
        let mut lexer = PklToken::lexer(self.source);

        let parsed = parse_pkl(&mut lexer)?;
        self.table = ast_to_hashmap(parsed)?;
